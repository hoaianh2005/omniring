...
/* Mảng dữ liệu Vi mã (Config File) 8KB chuẩn từ Bosch BMI270 */
const uint8_t bmi270_config_file[BMI270_CONFIG_FILE_SIZE] = {
...
{
    if (!dev || !dev->data) return -1;
    const struct i2c_dt_spec* i2c_dev = (const struct i2c_dt_spec*)dev->data;
    
    return i2c_reg_read_byte_dt(i2c_dev, reg_addr, data);
}
...
int bmi270_reg_write(const struct device_t* dev, uint8_t reg_addr, uint8_t data)
{
    ...
    return i2c_reg_write_byte_dt(i2c_dev, reg_addr, data);
}
...
int bmi270_init(const struct device_t* dev)
{
    ...
    printk("BMI270: I2C ready, addr=0x%02X\n", i2c_dev->addr);

    // 1. Soft Reset cảm biến
    ret = bmi270_reg_write(dev, BMI270_REG_CMD, BMI270_CMD_SOFT_RESET);
    if (ret < 0) return ret;
    k_msleep(10); // Chờ quá trình Power-On Reset hoàn tất

    // 2. Kiểm tra CHIP ID
    ret = bmi270_reg_read(dev, BMI270_REG_CHIP_ID, &chip_id);
    if (ret < 0 || chip_id != BMI270_CHIP_ID_VAL) {
        printk("BMI270: Invalid Chip ID (0x%02X)\n", chip_id);
        return -2;
    }

    // 3. Tắt Advance Power Save trước khi load Config File
    ret = bmi270_reg_write(dev, BMI270_REG_PWR_CONF, 0x00);
    if (ret < 0) return ret;
    k_usleep(450);

    // 4. Bắt đầu quy trình nạp Config File 8KB vào ASIC
    ret = bmi270_reg_write(dev, BMI270_REG_INIT_CTRL, 0x00);
    if (ret < 0) return ret;

    // BỔ SUNG: Trỏ địa chỉ RAM về 0x0000
    ret = bmi270_reg_write(dev, 0x5B, 0x00);
    if (ret < 0) return ret;
    ret = bmi270_reg_write(dev, 0x5C, 0x00);
    if (ret < 0) return ret;

    // Ghi 8192 bytes vào thanh ghi INIT_DATA theo từng chunk 128 bytes để tránh tràn bộ đệm I2C/DMA
    uint32_t chunk_size = 128;
    for (uint32_t i = 0; i < BMI270_CONFIG_FILE_SIZE; i += chunk_size) {
        ret = i2c_burst_write(i2c_dev->bus, i2c_dev->addr, BMI270_REG_INIT_DATA, &bmi270_config_file[i], chunk_size);
        if (ret < 0) return ret;
        k_usleep(100); // Cho I2C nghỉ 1 chút giữa các chunk
    }

    // 5. Xác nhận hoàn tất quá trình nạp
    ret = bmi270_reg_write(dev, BMI270_REG_INIT_CTRL, 0x01);
    if (ret < 0) return ret;
    k_msleep(20); // Chờ ASIC kiểm tra vi mã (Initialization sequence)

    // 6. Kiểm tra trạng thái nạp thành công (INTERNAL_STATUS[3:0] == 0x01)
    uint8_t status = 0;
    ret = bmi270_reg_read(dev, BMI270_REG_INTERNAL_STATUS, &status);
    if (ret < 0) return ret;
    if ((status & 0x0F) != 0x01) {
        printk("BMI270: Microcode load failed! Status: 0x%02X\n", status);
        return -3;
    }

    // 7. Bật nguồn Accel & Gyroscope trong PWR_CTRL
    bmi270_reg_write(dev, BMI270_REG_PWR_CTRL, 0x0E); // Bit 2: Accel, Bit 3: Gyro
    
    printk("BMI270 initialized successfully.\n");
    return 0;
}
...
int bmi270_config(const struct device_t* dev)
{
    ...
    // Cấu hình Accel: ODR = 100Hz, Normal mode
    ret = bmi270_reg_write(dev, BMI270_REG_ACC_CONF, 0xA8);
    if (ret < 0) return ret;
    
    // Cấu hình FIFO: Enable Accel FIFO & Enable Header Mode (Header 0x84)
    ret = bmi270_reg_write(dev, BMI270_REG_FIFO_CONFIG_0, 0x00);
    if (ret < 0) return ret;
    ret = bmi270_reg_write(dev, BMI270_REG_FIFO_CONFIG_1, 0x50);
    if (ret < 0) return ret;
...
{
    if (!dev || !dev->data || !data || !len_of_raw) return -1;
    const struct i2c_dt_spec* i2c_dev = (const struct i2c_dt_spec*)dev->data;

    uint8_t len_bytes[2] = {0};
    int ret = i2c_burst_read(i2c_dev->bus, i2c_dev->addr, BMI270_REG_FIFO_LENGTH_0, len_bytes, 2);
    if (ret < 0) return ret;

    uint32_t fifo_len = ((uint32_t)len_bytes[1] << 8) | len_bytes[0];
    if (fifo_len == 0) {
        *len_of_raw = 0;
        return 0;
    }

    // Đọc toàn bộ gói dữ liệu thô trong FIFO
    ret = i2c_burst_read(i2c_dev->bus, i2c_dev->addr, BMI270_REG_FIFO_DATA, data, fifo_len);
    if (ret < 0) return ret;
...
{
    if (!raw || !result || !len_of_result || len_of_raw == 0) {
        if (len_of_result) *len_of_result = 0;
        return;
    }
    ...
    while (idx < len_of_raw) {
        if (raw[idx] == 0x84) { 
            // Header Mode (Accel Frame)
            if (idx + 7 <= len_of_raw) {
                result[count].x = (int16_t)((raw[idx + 2] << 8) | raw[idx + 1]);
                result[count].y = (int16_t)((raw[idx + 4] << 8) | raw[idx + 3]);
                result[count].z = (int16_t)((raw[idx + 6] << 8) | raw[idx + 5]);
                idx += 7;
                count++;
            } else {
                break;
            }
        } else if (raw[idx] == 0x80) {
            break;
        } else if ((raw[idx] & 0x80) != 0) {
            idx += 7; 
        } else {
            // Headerless Mode
            if (idx + 6 <= len_of_raw) {
                result[count].x = (int16_t)((raw[idx + 1] << 8) | raw[idx]);
                result[count].y = (int16_t)((raw[idx + 3] << 8) | raw[idx + 2]);
                result[count].z = (int16_t)((raw[idx + 5] << 8) | raw[idx + 4]);
                idx += 6;
                count++;
            } else {
                break;
...
int bmi270_sleep(const struct device_t* dev)
{
    ...
    // Tắt Accel và Gyroscope
    bmi270_reg_write(dev, BMI270_REG_PWR_CTRL, 0x00);
    // Kích hoạt chế độ Low Power Advance Power Save
    bmi270_reg_write(dev, BMI270_REG_PWR_CONF, 0x01);
